/**
 * The single source of truth for *where things sit* in the lab scene.
 *
 * Two render layers draw the bench: a static SVG apparatus layer (glass
 ...
 * glass on one device, a flame missing its beaker on another. So both layers
 ...
 * scene. Reproduces the renderer's original per-beaker geometry exactly, so the
 * fluid layer keeps painting in the same place once the glass moves to SVG.
 *
 * `count` is clamped to at least 1, so a single-station experiment is a row of
 * one (and reads larger than one column of three — the existing size rule).
 */
export function computeStationLayout(
  ...
  for (let i = 0; i < n; i++) {
    const cx = colW * (i + 0.5);
    // A single beaker reads large; a row of three sits tighter.
