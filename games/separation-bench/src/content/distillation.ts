/**
 * Distillation of a miscible mixture (acetone + water), authored purely as data
 * on the workspace schema. This is the forcing experiment for the separation
 * toolkit's newest capability: the `distil` transform, which recovers a boiled-
 * off liquid instead of losing it as vapour the way `evaporate` does.
 *
 * NCERT Class 9 / Grade 9 — "Exploring Mixtures and their Separation" §5.3.2.
 * A still pot (distillation flask) holds a mixture of acetone (boils at ~56 °C)
 * and water (boils at 100 °C). Heating it once drives the lower-boiling acetone
 * over the condenser first; it cools and drips into the first receiver as a pure
 * liquid, while the water stays behind. Heating the flask again now sends the
 * water over into the second receiver. Two clean liquids, recovered.
 *
 * Routing is by id, never hard-coded: the first rule names acetone as the
 * volatile cut, the second names water. First-match-wins picks the acetone rule
 * while both are present, then the water rule once only water is left.
 */

import type { Scenario } from "@learn-loop/core";

const MIXTURE = "#cfd8dc"; // cloudy acetone + water in the flask
const ACETONE = "#e9eef3"; // near-colourless, faint cool tint
...
export const distillationScenario: Scenario = {
  ...
  // No reagent shelf: distillation is a heat-only experiment.
  shelf: [],
  stations: {
    flask: {
    ...
    },
    {
      ...
      options: [
        ...
        },
        {
          ...
          correct: false,
          feedback:
            "Not quite — cooling won't move the water out of the flask. It has " +
            "to be boiled over.",
        },
        {
      ...
      explanation:
        ...
        "in the second receiver — both liquids are now recovered, pure and " +
        "apart.",
    },
  ],
};
